...
def open_popup():
    ...
    topR.lift(rootzg1)
    rootzg1_x = rootzg1.winfo_rootx()
    rootzg1_y = rootzg1.winfo_rooty()
    win_x = rootzg1_x + 300
    win_y = rootzg1_y + 100
...
def open_help_popup():
    topHelp= tkinter.Toplevel(rootzg1)
    topHelp.geometry("1250x180")
    topHelp.resizable(width=False,height=False)
    topHelp.title("How To Play?")
    topHelp.transient(rootzg1)
    topHelp.lift(rootzg1)
    rootzg1_x = rootzg1.winfo_rootx()
    rootzg1_y = rootzg1.winfo_rooty()
    win_x = rootzg1_x + 300
    win_y = rootzg1_y + 100
    topHelp.geometry(f'+{win_x}+{win_y}')
    tkinter.Label(topHelp, text= "Each turn, a player repeatedly rolls a die until either a 1 is rolled or the player decides to hold: \nIf the player rolls a 1, they score nothing and it becomes the next player's turn. \nIf the player rolls any other number, it is added to their turn total and the player's turn continues. \nIf a player chooses to hold, their turn total is added to their score, and it becomes the next player's turn. \nThe first player to score 100 or more points wins.", font=('Mistral 18 bold'),
